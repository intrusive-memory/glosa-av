...
struct GlosaCompilerTests {
  ...
  func requirementsSection31FullExample() throws {
    ...
    let dialogueLines: [(character: String, text: String)] = [
      ...
      (
        ...
        text: #"Key-value pairs. Right now the only one that matters is "instruct.""#
      ),
      (
        character: "THE PRACTITIONER",
        text: "I need a translator. A layer that sits between the score and the model."
      ),
  ...
  func missingAttributesDiagnostics() throws {
    ...
    let missingToDiagnostics = result.diagnostics.filter {
      $0.message.contains("'to'")
        || $0.message.lowercased().contains("missing") && $0.message.contains("to")
    }
  ...
  func steamRoomFullEndToEnd() throws {
    ...
    let dialogueLines: [(character: String, text: String)] = [
      ...
      (
        character: "KILLIAN",
        text: "Insulin. You need to give him a mega dose of the fast acting stuff."
      ),
      (character: "BERNARD", text: "Yeah, but doesn't that take a few minutes--"),
      (
        character: "KILLIAN",
        text: "He needs to be far enough away from anyone or anything that can help him."
      ),
      (character: "BERNARD", text: "Where would that be?"),
      (
        character: "KILLIAN",
        text: "When he goes running. His elevated heart rate will make the mega dose more potent."
      ),
      (character: "BERNARD", text: "He runs marathons. How am I going to keep up with him?"),
      (
        character: "KILLIAN",
        text: "You don't have to keep up with him. What you have to do is attract him."
      ),
  ...
  func instructProvenanceCodable() throws {
    let prov = InstructProvenance(
      ...
      composedInstruct:
        "Night in room, rain. Approaching angry from calm, fast pace. Nervous. Ceiling: moderate."
    )
