...
struct GlosaSerializerFDXTests {
  ...
  func fdxMarkerIntentUsesSelfClosingTag() throws {
    ...
    #expect(
      xmlString.contains("<glosa:Intent from=\"frustrated\" to=\"resolved\""),
      "Missing marker intent element")

    // Verify the scoped intent has opening and closing tags.
    #expect(
      xmlString.contains("</glosa:Intent>"),
  ...
  private func buildScreenplayFromFDXContent() -> GuionParsedElementCollection {
    let elements: [GuionElement] = [
      ...
      GuionElement(
        elementType: .dialogue, elementText: "I've been staring at this struct for an hour."),
      ...
      GuionElement(
        ...
        elementText: "I need a translator. A layer that sits between the score and the model."),
      GuionElement(elementType: .character, elementText: "ESPECTRO FAMILIAR"),
      GuionElement(
        elementType: .dialogue, elementText: "Now you are thinking like a language designer."),
    ]
  ...
  private func buildNotesFromScore(
    ...
    for scene in score.scenes {
      ...
      var sceneTag =
        "<SceneContext location=\"\(scene.context.location)\" time=\"\(scene.context.time)\""
      ...
      for (intentIndex, intentEntry) in scene.intents.enumerated() {
        ...
        // Intent opening.
        var intentTag =
          "<Intent from=\"\(intentEntry.intent.from)\" to=\"\(intentEntry.intent.to)\""
