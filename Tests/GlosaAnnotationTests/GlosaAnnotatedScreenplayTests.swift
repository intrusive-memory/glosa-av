...
struct GlosaAnnotatedScreenplayTests {
  ...
  private func makeCompilationResult() -> CompilationResult {
    let sceneCtx = SceneContext(
      location: "the study", time: "late night", ambience: "quiet hum of electronics")
    let intent = Intent(
      from: "curious", to: "frustrated", pace: "moderate", spacing: nil, scoped: true, lineCount: 2)
    let constraint = Constraint(
      character: "DR. CHEN", direction: "thinking aloud", register: nil, ceiling: "moderate")
    ...
    let provenance: [InstructProvenance] = [
      InstructProvenance(
        ...
        composedInstruct:
          "Late night in the study, quiet hum of electronics. Curious, early in arc toward frustrated, moderate pace. Thinking aloud. Ceiling: moderate."
      ),
      InstructProvenance(
        lineIndex: 1,
        characterName: "DR. CHEN",
        ...
        constraint: constraint,
        composedInstruct:
          "Late night in the study, quiet hum of electronics. Frustrated, deep into arc from curious, moderate pace. Thinking aloud. Ceiling: moderate."
      ),
    ]
  ...
  func instructMatchesCompilationResultForAllDialogue() {
    ...
    for annotatedElement in annotated.annotatedElements {
      if annotatedElement.element.elementType == .dialogue {
        ...
        #expect(
          annotatedElement.instruct == expectedInstruct,
